...
// Pure-функция, которую можно вызывать вне React-цикла (например, из бенчмарка).
export function computeSupplierFuzzyResult(s: Supplier): SupplierFuzzyResult {
...
export function useSupplierRules(): SupplierFuzzyResult[] {
  ...
  return useMemo(() => supplier.map(computeSupplierFuzzyResult), [supplier]);
}
